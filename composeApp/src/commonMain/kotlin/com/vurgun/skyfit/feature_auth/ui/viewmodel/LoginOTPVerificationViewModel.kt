...
class LoginOTPVerificationViewModel(
    ...
    fun onConfirmClicked() {
        ...
        viewModelScope.launch {
            ...
            try {
                _events.emit(
                    when (val result = authRepository.verifyOTP(_enteredOtp.value)) {
                        is AuthOTPResult.Error -> LoginOTPVerificationViewEvent.ShowError(result.message)
                        AuthOTPResult.RegisterSuccess -> LoginOTPVerificationViewEvent.GoToRegister
                        AuthOTPResult.LoginSuccess -> LoginOTPVerificationViewEvent.GoToDashboard
                    }
                )
            } finally {
